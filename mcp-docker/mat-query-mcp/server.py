...
from tools import (search_crystal_structures_from_materials_project,
...
def main(port: int, transport: str = 'sse') -> int:
    ...
    """
    # 验证配置
    mp_valid = mp_config.validate_config()
    server_valid = server_config.validate_config()

    if not mp_valid:
        logger.warning("MP API 配置验证失败，但服务器将继续启动")
    if not server_valid:
        logger.warning("服务器配置验证失败，但服务器将继续启动")

    logger.info(f"启动MAT-QUERY-MCP服务器，传输类型: {transport}, 端口: {port}")
    ...
    if transport == "sse":
        ...
        async def handle_root(request):
            ...
            return JSONResponse({
                ...
                "description": "Materials Query MCP Server",
