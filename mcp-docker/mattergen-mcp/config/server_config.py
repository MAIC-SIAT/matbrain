...
class ServerConfig:
    """MatterGen MCP 服务器配置类"""

    def __init__(self):
        # 服务器配置
        self.SERVER_HOST: str = os.getenv("SERVER_HOST", "0.0.0.0")
        self.SERVER_PORT: int = int(os.getenv("SERVER_PORT", "5670"))  # 使用不同端口避免与其他 MCP 服务冲突

        # 工具调用超时时间（秒）- MatterGen 生成需要较长时间
        self.TOOL_CALL_TIMEOUT: int = int(os.getenv("TOOL_CALL_TIMEOUT", "360"))
